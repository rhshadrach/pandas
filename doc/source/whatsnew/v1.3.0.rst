...
- :meth:`DataFrame.aggregate`, :meth:`.DataFrameGroupBy.aggregate`, :meth:`Series.aggregate`, and :meth:`.SeriesGroupBy.aggregate` with dict-like arguments will now successfully return if part of their results raise a ``TypeError``. This is now consistent with the behavior for list-like arguments (:issue:`40539`)
